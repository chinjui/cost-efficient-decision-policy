...
def traj_segment_generator(sub_policy, env, macrolen, horizon, stochastic, args, sub_policy_costs, fixed_policy=None):
    episode_count = 0
    replay = args.replay
    t = 0
    ...
    rews_without_cost = np.zeros(horizon, 'float32')
    ...
    while True:
        ...
        ac, vpred = sub_policy.act(stochastic, ob)
        ...
        if t > 0 and t % horizon == 0:
            ...
            # print(total)
            # total = [0,0]
            dicti = {"ob" : obs, "rew" : rews, "vpred" : vpreds, "new" : news, "ac" : acs, "ep_rets" : ep_rets, "ep_lens" : ep_lens, "macro_ac" : macro_acs, "macro_vpred" : macro_vpreds, "ep_rets_without_cost": ep_rets_without_cost, "rews_without_cost": rews_without_cost}
            if len(rgb_arrays) != 0:
                dicti['rgb_arrays'] = rgb_arrays
        ...
        if replay:
            if len(ep_rets) == 0:
                ...
                env.render()
                    # print(info)
            pass
        if episode_count >= (args.train_time * args.begin_save_img) and episode_count % args.save_img_interval == 0 and args.save_img:
